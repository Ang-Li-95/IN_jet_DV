...
def importData(split, normalize=True,shuffle=True):
    ...
    data_train[3] = data_train[0]
    data_val[3] = data_val[0]
    data_test[3] = data_test[0]
...
out = phi_output(P)
out_sigmoid = tf.math.sigmoid(out)
...
Rr_train, Rs_train, Ra_train = getRmatrix_dR2(train[3][:,1:3,:])
Rr_val, Rs_val, Ra_val = getRmatrix_dR2(val[3][:,1:3,:])
...
for i in range(num_epochs):
    ...
    for j in range(int(len(train[0])/batch_num)):
        ...
        l_train,_,bce_train,dcorr_train=sess.run([loss,trainer,loss_bce,dcorr],feed_dict={O:batch_data,Rr:batch_Rr,Rs:batch_Rs,Ra:batch_Ra,label:batch_label,ntk_max:batch_ntk,evtweight:batch_weight})
        loss_train+=l_train
    ...
    for j in range(int(len(val[0])/batch_num)):
        ...
        l_val,_,bce_val,dcorr_val=sess.run([loss,out_sigmoid,loss_bce,dcorr],feed_dict={O:batch_data,Rr:batch_Rr,Rs:batch_Rs,Ra:batch_Ra,label:batch_label,ntk_max:batch_ntk,evtweight:batch_weight})
        loss_val+=l_val
        l_bce_val+=bce_val
...
with tf.Session() as newsess:
    ...
    Rr_test, Rs_test, Ra_test = getRmatrix_dR2(test[3][:,1:3,:])
    #weight_test = (test[2]-1)*(-1)
    weight_test = np.ones(test[2].shape)
    a,b = newsess.run([loss,out_sigmoid],feed_dict={O:test[0],Rr:Rr_test,Rs:Rs_test,Ra:Ra_test,label:test[2],ntk_max:test[1],evtweight:np.ones(test[2].shape)}) #FIXME: testing event weight not changed (test[2]-1)*(-1)
