...
fndir = '/uscms/home/ali/nobackup/LLP/crabdir/MLTreeAllVtxVkeeptk_v1METm/'
...
def GetDataAndLabel(fns, split, isSignal, cut="(met_pt >= 150) & (max_SV_ntracks > 2)", lumi=50000):
    tk_train = []
    tk_val = []
    tk_test = []
    vtx_train = []
    ...
    for fn in fns:
        ...
        # apply cuts
        evt_select = (matrix['met_pt']>=150) & (matrix['vtx_ntk']>2) & (matrix['vtx_dBVerr']<0.01)
        for v in matrix:
...
for i in range(num_epochs):
    ...
    for j in range(int(len(train[0])/batch_num)):
        ...
        batch_vtx = train[1][j*batch_num:(j+1)*batch_num][:,1:]
        ...
        l_train,_,bce_train,dcorr_ntk_train=sess.run([loss,trainer,loss_bce,dcorr],feed_dict={O:batch_tk,Rr:batch_Rr,Rs:batch_Rs,Ra:batch_Ra,vtx:batch_vtx,label:batch_label,ntk_max:batch_ntk,evtweight:batch_weight})
        loss_train+=l_train
    ...
    for j in range(int(len(val[0])/batch_num)):
        ...
        batch_vtx = val[1][j*batch_num:(j+1)*batch_num][:,1:]
        ...
        l_val,_,bce_val,dcorr_ntk_val=sess.run([loss,out_sigmoid,loss_bce,dcorr],feed_dict={O:batch_tk,Rr:batch_Rr,Rs:batch_Rs,Ra:batch_Ra,vtx:batch_vtx,label:batch_label,ntk_max:batch_ntk,evtweight:batch_weight})
        loss_val+=l_val
        l_bce_val+=bce_val
...
with tf.Session() as newsess:
    ...
    for j in range(int(len(test[0])/batch_num)+1):
        ...
        batch_vtx = test[1][j*batch_num:next_idx][:,1:]
